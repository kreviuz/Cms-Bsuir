...
namespace Diploma.DataHelpers
{
    public static partial class DtoHelper
    {
        ...
        public static IEnumerable<PathModel> GetPaths()
        {
            using (var xmlSteam = new FileStream(HttpContext.Current.Server.MapPath("~/App_Data/Paths/PagePaths.xml"), FileMode.Open))
            {
                var serializer = new XmlSerializer(typeof(List));
                var dto = (List)serializer.Deserialize(xmlSteam);
                var items = dto.Path.Select(GetPaths);
                return items;
            }
        }
